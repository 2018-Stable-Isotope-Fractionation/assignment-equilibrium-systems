...
Starting with the exact mass balance equation below, show analytically what the error introduced by the mass-balance in $\delta$ space introduces.
...
Based strictly on the numbers provided above, what percentage of the ethanol would you estimate to be potentially derived from the lower quality ingredient for each of the 3 new products? Store the result in column `f_potato` in a data frame named `df_2b` and comment on what you would tell the importer about the quality of their 3 new products.

```{r}

```

> Solution: Product A is most likely authentic (i.e. all derived from sugarcane) with the isotopic composition below the median for C4 plants wherease products B and C are suspicious with their isotopic composition higher than the median for C4 plants. Specifically, products B and C are suspicious with potentially ~37.7% of the ethanol in B and 10% in C derived from potato instead.
...
$$
\ln \, \alpha_{CC/H2O} = 2.78 \cdot 10^6 \frac{1}{T^2} - 2.89
$$
